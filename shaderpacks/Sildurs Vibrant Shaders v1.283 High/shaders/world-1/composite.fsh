...
vec4 data = texture2D(colortex0,texcoord);
vec4 dataUnpacked0 = vec4(decodeVec2(data.x),decodeVec2(data.y));
vec4 dataUnpacked1 = vec4(decodeVec2(data.z),decodeVec2(data.w));
vec3 normalT = mat3(gbufferModelViewInverse) * decode(dataUnpacked0.yw);

vec3 albedo = vec3(dataUnpacked0.xz,dataUnpacked1.x);
vec3 color = pow(albedo.rgb,vec3(2.2));
vec2 lightmap = dataUnpacked1.yz; //x=emissive, y=sky

bool sky = (texture2D(depthtex0, texcoord).x >= 1.0);
...
#else
vec4 TAAfragpos = gbufferProjectionInverse * (vec4(texcoord,Depth,1.0) * 2.0 - 1.0);
TAAfragpos /= TAAfragpos.w;
#endif
...
vec4 fragpos = gbufferProjectionInverse * (vec4(texcoord,Depth,1.0) * 2.0 - 1.0);
	...
	float occlusion = calcSSDO(fragpos.xyz, decode(dataUnpacked0.yw));
	ao = pow(1.0-occlusion, ao_strength);
}
#endif
	...
	float finalminlight = (nightVision > 0.9)? 0.15 : (minlight+0.025)+(screenBrightness*0.05); //add nightvision support but make sure minlight is still adjustable. Also scale with brightness slider
