...
bool sky = (texture2D(depthtex1, texcoord).x >= 1.0);
...
vec4 fragpos = gbufferProjectionInverse * (vec4(texcoord,Depth,1.0) * 2.0 - 1.0);
	...
	float finalminlight = (nightVision > 0.9)? 0.15 : (minlight+0.006)+(screenBrightness*0.0125); //add nightvision support but make sure minlight is still adjustable.	
