...
if (!sky){
//Water and Ice
vec3 Wnormal = texture2D(colortex1,texcoord).xyz;
...
float ao = 1.0;
#ifdef SSDO
	float occlusion = calcSSDO(TAAfragpos, decode(dataUnpacked0.yw));
	...
	float finalminlight = (nightVision > 0.9)? 0.15 : minlight; //add nightvision support but make sure minlight is still adjustable.	
	...
	float diffuse = translucent? 0.75 : clamp(dot(normal, shadowLightpos),0.0,1.0);	
