...
bool isPolished = abs(dataUnpacked1.w-0.85) < 0.01;
bool isMetallic = abs(dataUnpacked1.w-0.95) < 0.01;
#ifndef polishedRefl
	isPolished = false;
#endif	
#ifndef metallicRefl
...
#ifdef Refraction
if (texture2D(colortex1, texcoord).z < 0.2499 && dot(texture2D(colortex1,texcoord).xyz,texture2D(colortex1,texcoord).xyz) > 0.0 || isEyeInWater == 1.0) { //reflective water
...
//Render before transparency
	...
	if (isMetallic || isPolished) {
		vec3 relfNormal = normalize(decode(dataUnpacked0.yw));
...
#ifdef RainReflections
	vec3 wnormal = normalize(decode(dataUnpacked0.yw));
